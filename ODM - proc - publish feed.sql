...
BEGIN
    ...
    DECLARE @span_policy        varchar(20),
            ...
            @excl_duplicate     char(1),
            @excl_hcp           varchar(200),
            @req_cin_extended   char(1),
            @excl_plan          varchar(200),
    ...
    BEGIN TRY
        ...
        SELECT  @span_policy        = span_policy,
                ...
                @excl_duplicate     = exclude_duplicate_flag,
                @excl_hcp           = exclude_hcp_list,
                @req_cin_extended   = require_cin_extended_flag,
                @excl_plan          = exclude_benefit_plan_list,
                @file_type          = file_type
        ...
        LEFT JOIN odm.enrollment_member_attribute dup
              ON  dup.member_identifier = s.member_identifier
        ...
        WHERE
          ...
          AND (@excl_hcp IS NULL
               OR @hcp_excl_list NOT LIKE '%,' + ISNULL(m.hcp_number,'~none~') + ',%')

          AND (@excl_plan IS NULL
               OR @plan_excl_list NOT LIKE '%,' + ISNULL(s.benefit_plan_identifier,'~none~') + ',%')

          AND (@req_cin_extended <> 'Y'
               OR EXISTS (SELECT 1 FROM odm.enrollment_identifier ce
