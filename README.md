/* 834 outbound -- publication. Takes the odm layer and writes one trading partner's file content.

   WHY THIS IS SEPARATE FROM SLICE 1 AND SLICE 2
   The publication filters read odm.enrollment_provider, odm.enrollment_compliance_program and
   odm.enrollment_member_attribute, which slice 2 loads. Publishing inside slice 1 meant
   direct_member_flag and wellrec_scope could never be computed, because their source tables
   were empty at that point. Both were sitting at their defaults.

   It also matches how the layers actually run. odm is a daily full refresh. pub runs per
   trading partner, on whatever cadence that partner's contract requires. Publishing NorthBay
   is now seconds rather than a full reload.

   RUN ORDER:  usp_load_slice_1  ->  usp_load_slice_2  ->  usp_publish_feed (once per feed)

   A Sharma, 24 Sep 2026. */
...
CREATE OR ALTER PROCEDURE pub.usp_publish_feed
...
-- FROM   ctl.load_run WHERE run_identifier > (SELECT MAX(run_identifier) - 4 FROM ctl.load_run)
